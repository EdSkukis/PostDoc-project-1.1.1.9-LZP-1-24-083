...
def run_friedman_analysis(
    ...
    try:
        ...
        for filename in os.listdir(input_dir):
            ...
            if not all(col in df.columns for col in ['T_K', 'inv_T_K', 'alpha_percent', 'dalpha_dt', 'ln_dalpha_dt']):
                continue
        ...
        for alpha_target in alpha_levels:
            ...
            for beta, df in sorted(data.items()):
                ...
                valid = df.dropna(subset=['ln_dalpha_dt'])
                if valid.empty:
                    continue
                idx = (valid['alpha'] - alpha_target).abs().idxmin()
                row = valid.loc[idx]
            ...
            y = np.array([r['ln_da_dt'] for r in rows])
        ...
        files_saved = {}
        if save_csv:
            ea_path = os.path.join(output_dir, f'{sample_name}_friedman_Ea.csv')
            ea_df.to_csv(ea_path, index=False, float_format='%.6f')
            files_saved['ea_csv'] = ea_path

            points_path = os.path.join(output_dir, f'{sample_name}_friedman_points.csv')
        ...
        if save_plots:
            ...
            for i, (alpha, pts) in enumerate(sorted(plot_data.items())):
                if len(pts['1/T']) < 2:
                    continue
                color = colors[i]
                plt.scatter(pts['1/T'], pts['ln_da_dt'], color=color, label=f'α = {alpha:.2f}')
                res = linregress(pts['1/T'], pts['ln_da_dt'])
                plt.plot(pts['1/T'], res.slope * np.array(pts['1/T']) + res.intercept, '--', color=color, alpha=0.7)
            plt.xlabel('1/T (K⁻¹)')
            plt.ylabel('ln(da/dt)')
            plt.title(f'Friedman Plot — {sample_name}')
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small')
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            lines_path = os.path.join(output_dir, f'{sample_name}_friedman_lines.png')
            ...
            plt.plot(ea_df['alpha'], ea_df['Ea_kJ_mol'], 'o-', color='darkred')
            plt.xlabel('Conversion degree α')
            plt.ylabel('Ea (kJ/mol)')
            plt.title(f'Activation Energy vs Conversion — {sample_name} (Friedman)')
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            ea_plot_path = os.path.join(output_dir, f'{sample_name}_friedman_Ea_vs_alpha.png')
